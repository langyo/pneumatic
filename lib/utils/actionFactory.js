export default ({ creator, translator, executor }) => {
  // Arguments' check
  ...
  return {
    creator: (...args) => {
      let availableFunc = creator
        .map(({ paras, func }, id) => ({ paras, id, func }))
        .filter(({ paras }) => args.length === paras.length)
        .filter(({ paras }) => {
          for (let i = 0; i < args.length; ++i) {
            if (typeof paras[i] === 'string' && paras[i] !== typeof args[i]) return false;
            else if (typeof paras[i] !== 'string' && args[i].__proto__ !== paras[i].__proto__) return false;
          }
          return true;
        });
      ...
      return { ...availableFunc[0].func(...args), $$id: availableFunc[0].id };
    },
    translator: translator ? Object.keys(translator).reduce((reduced, key) => ({ ...reduced, [key]: obj => translator[key][obj.$$id](obj) }), {}) : {},
    executor: Object.keys(executor).reduce((reduced, key) => ({ ...reduced, [key]: obj => executor[key][obj.$$id](obj) }),{})
  }
};