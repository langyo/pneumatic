...
export const loadActionModel = ({ $packageName: packageName, $actions: actions }) => {
  ...
  if (packageName === 'preset') {
    ...
    for (let key of Object.keys(actions)) {
      ...
      clientActionsTranslator[key] = actions[key].translator && actions[key].translator.client || (p => [p]);
      serverActionsTranslator[key] = actions[key].translator && actions[key].translator.server || (p => [p]);
      nativeActionsTranslator[key] = actions[key].translator && actions[key].translator.native || (p => [p]);
    ...
    for (let key of Object.keys(actions)) {
      ...
      clientActionsTranslator[`${packageName}.${key}`] = actions[key].translator && actions[key].translator.client || (p => [p]);
      serverActionsTranslator[`${packageName}.${key}`] = actions[key].translator && actions[key].translator.server || (p => [p]);
      nativeActionsTranslator[`${packageName}.${key}`] = actions[key].translator && actions[key].translator.native || (p => [p]);
